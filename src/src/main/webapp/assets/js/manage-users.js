 ...
 * @author IT24102137
 * @version 2.2.0
 * Last Updated: 2025-03-26 11:00:09
 */
...
function updateUserCountsFromTable() {
    ...
    document.getElementById('currentResults').textContent = totalUsers;
...
function attachRowEventHandlers(row) {
    ...
    if (viewBtn) {
        viewBtn.addEventListener('click', function() {
            ...
            fetch(getContextPath() + '/UserServlet/' + userId)
                .then(response => {
                    if (!response.ok) {
